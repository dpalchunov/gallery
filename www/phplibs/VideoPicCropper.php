<?php
class VideoPicCropper extends PicCropper
{

    private $thumbDir = 'videos/thumbs';
    private $fileName;
    private $thumb;
    private $dst_h;
    private $dst_w;
    ...
    function VideoPicCropper($src, $data, $dst_w, $dst_h)
    {
        $this->srcDir = 'uploads/vid_tmp';
        $this->dstDir = 'videos/thumbs';
    ...
    public function crop()
    {
        ...
        if (!empty($src) && !empty($dst) && !empty($data)) {
            ...
            if ($src_part_w <= $w_limit and $src_part_h <= $h_limit) {
                $thumb_w = $src_part_w;
                $thumb_h = $src_part_h;
            } else {
                $ratio = $src_part_w / $src_part_h;
                if ((($src_part_w) / $w_limit) >= ($src_part_h) / $h_limit) {
                    $thumb_w = $w_limit;
